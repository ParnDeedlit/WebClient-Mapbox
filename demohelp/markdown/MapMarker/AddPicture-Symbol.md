...
> 请特别关注 `"icon-image": "college-15"`, 它与line-pattern,fill-pattern一样是对应样式库中的图片的名字，样式库sprite在下面有详细讲解。`特别重要，与上面的Marker添加图片的方式不同的是，这里的添加的图片不能是随意添加的图片，只能是一套完整的样式库图片`,优点在于该图片的表现形式和分辨率都完成传统的png方式
